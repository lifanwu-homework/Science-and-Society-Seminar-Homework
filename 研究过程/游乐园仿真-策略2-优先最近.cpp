...
}s[spot_number]={spot(100,22,100,1),spot(3,50,20,5),spot(4,300,40,12),spot(200,250,10,3),spot(100,400,50,15)};
		...
		void decision()
		{
			...
			if(this->situation==visit)
			{
				if(this->lasttime>=s[this->location].playtime)
				{
					...
					//找出下一个参观点，策略是优先最近的未参观点 
					int i,j=0,min;
					for(i=0,min=100000;i<spot_number;i++)
					{
					if(s[this->location].Distance(s[i])<min&&this->visited[i]==0)
					{
					min=s[this->location].Distance(s[i]);
					j=i;
					}
					}
					/*if(j==0)
					{
					for(j=0;this->visited[j]!=0;j++);
					}*/
					
					
					this->situation=j;//参观策略 
					s[this->location].all_people--;
					s[this->location].people--;
					this->lasttime=0;
...
class LinkList{
	public:
		...
		void people_generate()//不同的人员产生方法 
		{
			int num;
			if(rand()%10>7)
			for(num=15;num;num--)
